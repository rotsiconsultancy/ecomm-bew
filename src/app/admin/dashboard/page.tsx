...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Header ────────────────────────────────────────────────────────── */}
      <div>
        <h1 className="text-3xl font-extrabold text-[#061f3f]">Dashboard Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <Card className="lg:col-span-3 rounded-2xl border-none shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-[#061f3f]">Recent Orders</h2>
            <Link
              href="/admin/order-management"
              className="text-xs font-semibold text-[#ff5f14] hover:underline"
            >
              View all →
            </Link>
          </div>
                    ...
                    <tr key={o.id} className="hover:bg-gray-50/60">
                      <td className="px-6 py-4">
                        <Link
                          ...
                          className="font-bold text-[#061f3f] hover:text-[#ff5f14] font-mono text-xs transition-colors"
                        >
                          {shortenId(o.id)}
                        </Link>
                      </td>
                      <td className="px-6 py-4 text-gray-700 font-medium">{o.customer_name}</td>
                      <td className="px-6 py-4 font-bold text-[#061f3f]">{formatCurrency(o.total_amount, o.currency)}</td>
                      <td className="px-6 py-4">
        ...
        <Card className="lg:col-span-2 rounded-2xl border-none shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-[#061f3f]">Recent Quotes</h2>
            <Link
              ...
              d.quotes.recent.map((q) => (
                ...
                >
                  <div className="min-w-0 flex-1 pr-3">
                    <p className="font-semibold text-[#061f3f] text-sm truncate">{q.full_name}</p>
                    {q.company && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6 rounded-2xl border-none shadow-sm">
          <h2 className="font-bold text-[#061f3f] mb-5">Order Status Breakdown</h2>
            ...
            {[
              ...
              <div key={label}>
                <div className="flex items-center justify-between text-sm mb-1.5">
                  ...
                  <span className="font-bold text-[#061f3f]">{count}</span>
                </div>
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between text-sm">
            ...
            <span className="font-bold text-[#061f3f]">{d.orders.this_month} orders</span>
          </div>
        </Card>
        ...
        <Card className="p-6 rounded-2xl border-none shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            <AlertTriangle className="w-4 h-4 text-[#ff5f14]" />
            <h2 className="font-bold text-[#061f3f]">Low Stock Alert</h2>
          </div>
                ...
                >
                  ...
                  <span
                    className={`text-xs font-extrabold shrink-0 ${
                      p.stock <= 5 ? 'text-red-500' : 'text-[#ff5f14]'
                    }`}
                  >
                    {p.stock} left
                  </span>
        ...
        <Card className="p-6 rounded-2xl border-none shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            <TrendingUp className="w-4 h-4 text-[#061f3f]" />
            <h2 className="font-bold text-[#061f3f]">Quick Stats</h2>
          </div>
          <div className="space-y-4">
            {[
              { icon: Users,       label: 'New users this month',  value: d.users.new_this_month,   color: 'bg-blue-50',   iconColor: 'text-blue-600' },
              { icon: Users,       label: 'Wholesale buyers',      value: d.users.wholesale,        color: 'bg-orange-50', iconColor: 'text-[#ff5f14]' },
              ...
              <div key={label} className="flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center justify-between">
                  ...
                  <span className="text-sm font-extrabold text-[#061f3f]">{value}</span>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-5 pt-4 border-t border-gray-100 grid grid-cols-2 gap-3 text-center">
            <div>
              <p className="text-xl font-extrabold text-[#061f3f]">{d.users.total}</p>
              <p className="text-xs text-gray-400 mt-0.5">Total users</p>
            </div>
            <div>
              <p className="text-xl font-extrabold text-[#061f3f]">{d.products.quote_only}</p>
