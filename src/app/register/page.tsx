...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    // TODO: TEMPORARY — assigning 'admin' role for dev/testing purposes.
    // Change to 'customer' (or remove this block entirely) before going to production.
    // The auth trigger in the DB inserts 'customer' by default; this override
    // upgrades the new user to admin so we can test protected admin routes.
    if (data.user) {
      await supabase
        .from('profiles')
        .upsert({ id: data.user.id, full_name: fullName, role: 'admin' })
    }
    ...
    // Email confirmation is enabled — show a message
    setSuccess(true)
    setLoading(false)
  }
  ...
  if (success) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f8f6f6] px-6">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#003366] flex-col justify-between p-12">
        ...
        <div className="space-y-4">
          <h2 className="text-white text-4xl font-bold leading-tight">
            Join Bewama<br />Today
          </h2>
          <p className="text-white/70 text-lg">
            Access wholesale pricing, track orders, and request quotes — all in one place.
          </p>
        </div>
